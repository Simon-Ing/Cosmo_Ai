...
//    std::cout << DATAPOINTS_TO_GENERATE << " " << window_size << " " << name << std::endl;
    ...
    for (einsteinR = window_size*0.0025; einsteinR <= window_size*0.125; einsteinR++) {
        for (source_size = window_size*0.0025; source_size <= window_size*0.125; source_size++) {
            for (xPos = window_size*0.25; xPos <= window_size*0.75; xPos++) {
                update(0, nullptr);
            }
        }
