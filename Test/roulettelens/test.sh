...
python3 $pyd/datagen.py -Z 600 --csvfile debug.csv --outfile roulette.csv --centred -D Original --actual | tee datagen.log || exit 1
...
do
   f=`basename $i`
   convert \( Original/actual-$f Original/$f +append \) \( diff/$f Roulette/$f +append \) -append montage/$f
done
